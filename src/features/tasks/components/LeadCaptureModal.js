// src/features/tasks/components/LeadCaptureModal.js
// Beautiful bottom-sheet modal to capture email + phone before unlocking partner link
...
export default function LeadCaptureModal({ visible, task, onSubmit, onClose }) {
  ...
  const validate = () => {
    ...
    if (!email.trim() || !/\S+@\S+\.\S+/.test(email)) {
      setEmailErr('Valid email zaroori hai'); ok = false;
    } else setEmailErr('');
    if (!mobile.trim() || !/^[6-9]\d{9}$/.test(mobile)) {
      setMobileErr('Valid 10-digit mobile number daalo'); ok = false;
    } else setMobileErr('');
    return ok;
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await onSubmit(email.trim(), mobile.trim());
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY: slideAnim }] }]}>
          ...
          <View style={styles.taskBanner}>
            ...
            <View style={styles.taskBannerInfo}>
              ...
              <View style={styles.creditPill}>
                <Text style={styles.creditPillText}>+{task.credits} Credits</Text>
              </View>
            </View>
          </View>
          ...
          <Text style={styles.subHeading}>
            Apna email aur mobile number submit karo — phir partner site ka link unlock ho jayega!
          </Text>
          ...
          <View style={[styles.inputWrap, mobileErr ? styles.inputError : null]}>
            ...
            <TextInput
              ...
              placeholderTextColor="#bbb"
              value={mobile}
              onChangeText={(t) => { setMobile(t); setMobileErr(''); }}
          ...
          <Text style={styles.privacyNote}>
            🔒 Aapka data secure hai. Sirf partner site ke liye use hoga.
          </Text>
          ...
          >
            {submitting
              ...
              : <Text style={styles.ctaBtnText}>Unlock Link & Earn {task.credits} Credits →</Text>
            }
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  creditPill: {
    ...
    backgroundColor: '#d4edda', borderRadius: 20,

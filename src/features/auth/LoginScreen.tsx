...
export default function LoginScreen({ navigation }: Props) {
  ...
  const [mfaStep, setMfaStep] = useState<string | null>(null);
  ...
  const getFirstPendingStep = (challenge: any, done: Set<string>): string | null => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const response = await GoogleSignin.signIn() as any; // ✅ FIX: cast to any for version compat
      console.log('✅ STEP 2: Response type:', response?.type);
      console.log('✅ STEP 2: Full response:', JSON.stringify(response));

      // ✅ FIX: Handle both v13+ (response.type) and older API shapes
      if (response?.type === 'cancelled' || response === null) {
        setLoading(false);
        return;
      }

      // ✅ FIX: Support both v13+ (response.data.idToken) and older (response.idToken)
      ...
      if (e?.code === statusCodes.SIGN_IN_CANCELLED) {
        ...
        setError(`[${e?.code ?? 'ERR'}] ${e?.nativeErrorMessage ?? e?.message ?? 'Google sign-in failed'}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  if (mfaData) {
    ...
    const stepIcon: Record<string, string> = { email: '📧', mobile: '📱', totp: '🔐' };
          ...
          <View style={styles.card}>
            ...
            <View style={styles.infoBox}>
              <Text style={styles.infoText}>
                {mfaStep ? stepIcon[mfaStep] : ''} {mfaStep ? stepLabel[mfaStep] : ''}
              </Text>
            </View>
